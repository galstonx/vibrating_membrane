...
direction, and then wave equation turns into a system of ODEs.
...
the forces acting on a point come from up/down/left/right neighbors

the index of the interior points is 0<= x < X_DIM, 0<= y < Y_DIM
the boundary indices are x=-1 ot X_DIM and y=-1 or Y_DIM
...
int main() {
  ...
  //
  // set up initial positions as a gaussian centered at middle
  int x_mid=X_DIM/2;
  int y_mid=Y_DIM/2;
  double scale_x=500.0/X_DIM/X_DIM;
  double scale_y=500.0/Y_DIM/Y_DIM;
  for(i=0;i<Y_DIM;++i) {
    for(j=0;j<X_DIM;++j) {
      ic[i*X_DIM+j]=4*exp(-scale_y*(i-y_mid)*(i-y_mid)-scale_x*(j-x_mid)*(j-x_mid));
    }
  }
