...
int main() {
  ...
  //
  // set up initial positions as two gaussians 
  int x_third=X_DIM/3;
  int y_mid=Y_DIM/2;
  ...
  for(i=0;i<Y_DIM;++i) {
    for(j=0;j<X_DIM;++j) {
      ic[i*X_DIM+j]=1*exp(-scale_y*(i-y_mid)*(i-y_mid)-scale_x*(j-x_third)*(j-x_third))+1*exp(-scale_y*(i-y_mid)*(i-y_mid)-scale_x*(j-2*x_third)*(j-2*x_third));
    }
  }
